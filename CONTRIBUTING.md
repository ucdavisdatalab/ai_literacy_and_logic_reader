...
The reader is written in Markdown and rendered with [Jupyter Book][JB]. To make
...
3.  Edit an existing chapter file or create a new one. Chapter files must be
    either Markdown files (`.md`) or Jupyter Notebook files (`.ipynb`). Put all
    chapter files in the `chapters/` directory. Enter your text, code, and
    other information directly into the file. Make sure your file:
...
When you build the course reader, Jupyter Book will run the Python code in the
chapters in the Pixi environment. We try to ensure that the Pixi environment
contains every package necessary, but if one is missing, Jupyter Book will
raise an error. Pay attention to error messages! If a package is missing, add
it to the Pixi environment (ask the maintainer if you need help with this).
